...
- Code **reuse**: child classes use the code in the parent class **without re-writing**, saves **time and effort**
...
Single variable can be used to store multiple data types. The user would be able to store several variables of different types (double, Float, Int, Long, etc) in a single variable. This would make it easier for the user to search for and implement these variables.
